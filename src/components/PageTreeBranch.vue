...
onMounted(() => {
  if (!container.value || props.parentKey === '__pageflow_favorites__') return
...
<template>
  <div ref="container" class="page-tree-branch" :data-parent-key="parentKey">
    <div v-for="node in nodes" :key="node.key" class="page-tree-item" :data-placement-key="placementKey(node)" :data-sortable="node.key !== 'group:__pageflow_favorites__' && node.parentKey !== '__pageflow_favorites__'">
      <div v-if="node.kind === 'group'" class="page-tree-row is-group" :class="{ 'is-active': node.key === activeGroupKey }" role="treeitem" :aria-expanded="expandedKeys.has(node.key)" :data-group-key="node.key" :style="{ '--tree-depth': depth }">
        ...
        <span class="page-tree-drag-handle" :class="{ 'is-virtual': node.key === 'group:__pageflow_favorites__' }"><svg class="page-tree-page-icon" viewBox="0 0 16 16"><path v-if="node.key === 'group:__pageflow_favorites__'" d="m8 2 1.8 3.7 4.1.6-3 2.9.7 4.1L8 11.4l-3.6 1.9.7-4.1-3-2.9 4.1-.6z" /><path v-else d="M2.5 4.5h4l1.2 1.5h5.8v6.5h-11z" /></svg><svg v-if="node.key !== 'group:__pageflow_favorites__'" class="page-tree-grip-icon" viewBox="0 0 10 14"><circle cx="3" cy="3" r="1" /><circle cx="7" cy="3" r="1" /><circle cx="3" cy="7" r="1" /><circle cx="7" cy="7" r="1" /><circle cx="3" cy="11" r="1" /><circle cx="7" cy="11" r="1" /></svg></span>
      ...
      <button v-else type="button" class="page-tree-row is-page" :class="{ 'is-active': node.pageId === activePageId, 'is-favorite': favoritePageIds?.has(node.pageId) }" :data-page-id="node.pageId" :style="{ '--tree-depth': depth }" :title="node.path" @click="emit('select', node.pageId)" @contextmenu="$event.preventDefault(); $event.stopPropagation(); emit('pageContextMenu', node.pageId, $event.clientX, $event.clientY)">
        <span class="page-tree-guide"></span><span class="page-tree-drag-handle" :class="{ 'is-virtual': node.parentKey === '__pageflow_favorites__' }"><svg class="page-tree-page-icon" viewBox="0 0 16 16"><path d="M4 2.5h5l3 3v8H4zM9 2.5v3h3" /></svg><svg v-if="node.parentKey !== '__pageflow_favorites__'" class="page-tree-grip-icon" viewBox="0 0 10 14"><circle cx="3" cy="3" r="1" /><circle cx="7" cy="3" r="1" /><circle cx="3" cy="7" r="1" /><circle cx="7" cy="7" r="1" /><circle cx="3" cy="11" r="1" /><circle cx="7" cy="11" r="1" /></svg></span>
