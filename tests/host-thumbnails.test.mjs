...
test('validates, replaces, and restores Chrome thumbnail cache records', async () => {
  ...
  try {
    ...
    const old = { pageId: '/a', mode: 'pc', revision: '1', source: 'data:old', width: 300, height: 600, updatedAt: 1 }
    const next = { ...old, revision: '2', source: 'data:new', updatedAt: 2 }
    assert.deepEqual(parseStoredHostThumbnails([old, { pageId: '/broken' }, null]), [old])
    assert.deepEqual(upsertStoredHostThumbnail([old], next), [next])
    assert.deepEqual(storedHostThumbnailRecords(next).map(record => [record.slot, record.revision, record.height]), [
      ['pc:compact:/a', '2', 600],
      ['pc:full:/a', '2', 600],
    ])
  } finally {
