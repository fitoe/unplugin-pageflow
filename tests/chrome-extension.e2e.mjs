...
test('Chrome extension smoke covers runtime, capture, diagnostics, workbench, and persistence', { timeout: 90_000 }, async () => {
  ...
  try {
    ...
    await dashboard.getByText('页面结构').waitFor()
    ...
    assert.deepEqual(restoredAboutWorldPosition, storedAboutWorldPosition)
    ...
    assert.equal(thumbnails.every(item => restoredThumbnails.some(restored => restored.pageId === item.pageId && restored.mode === item.mode)), true)
    assert.deepEqual(restoredThumbnails.filter(item => thumbnailTimes.has(`${item.pageId}:${item.mode}`)
      && thumbnailTimes.get(`${item.pageId}:${item.mode}`) !== item.updatedAt)
