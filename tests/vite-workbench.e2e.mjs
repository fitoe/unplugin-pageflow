...
test('Vite workbench smoke covers navigation, focus, viewport, theme, hotspots, diagnostics, todos, and the page tree', { timeout: 90_000 }, async () => {
  ...
  try {
    ...
    const focusedCheckout = page.locator('.page-preview[data-page-id="checkout"]')
    const focusedCheckoutBeforeRepeat = await focusedCheckout.boundingBox()
    assert(focusedCheckoutBeforeRepeat)
    await page.locator('.page-tree-row.is-page[aria-current="page"]').click()
    await page.waitForTimeout(400)
    const focusedCheckoutAfterRepeat = await focusedCheckout.boundingBox()
    assert(focusedCheckoutAfterRepeat)
    assert(Math.abs(focusedCheckoutAfterRepeat.x - focusedCheckoutBeforeRepeat.x) < 1)
    assert(Math.abs(focusedCheckoutAfterRepeat.y - focusedCheckoutBeforeRepeat.y) < 1)
    assert(Math.abs(focusedCheckoutAfterRepeat.width - focusedCheckoutBeforeRepeat.width) < 1)
