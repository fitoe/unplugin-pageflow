...
test('Vite workbench smoke covers navigation, focus, viewport, theme, hotspots, diagnostics, and todos', { timeout: 60_000 }, async () => {
  ...
  try {
    ...
    await page.mouse.move(checkoutBefore.x + checkoutBefore.width / 2, checkoutBefore.y + checkoutBefore.height / 2)
    await page.waitForTimeout(50)
    assert.equal(await page.locator('.canvas').evaluate(element => getComputedStyle(element).cursor), 'default')
    await page.mouse.move(checkoutBefore.x + checkoutBefore.width / 2, checkoutBefore.y + checkoutBefore.height + 20)
    await page.mouse.down()
    assert.notEqual(await page.locator('.canvas').evaluate(element => getComputedStyle(element).cursor), 'move')
    await page.mouse.move(checkoutBefore.x + checkoutBefore.width / 2 - 170, checkoutBefore.y + checkoutBefore.height + 120, { steps: 8 })
    assert.equal(await page.locator('.canvas').evaluate(element => getComputedStyle(element).cursor), 'move')
